...
{$IFDEF SYN_LAZARUS}
procedure TSynWebBase.GetTokenEx(out TokenStart: PChar;out TokenLength: integer);
begin
  TokenLength:=FInstance.FRun-FInstance.FTokenPos;
  TokenStart:= FInstance.FLine + FInstance.fTokenPos;
end;
{$ENDIF}
...
{$IFNDEF UNISYNEDIT}
procedure TSynWebBase.SetLine({$IFDEF FPC}const {$ENDIF}NewValue: String; LineNumber: Integer);
begin
  if FEngine = nil then
    Exit;
  FEngine.FInstance := @FInstance;
...
{$ENDIF}
 ...
 // FCssRulesetWhitespaceAttri.Background := clInfoBk;
  ...
  //FCssPropAttri.Background := clInfoBk;
  ...
  AddAttribute(FCssSymbolAttri);
   ...
   FCssErrorAttri.Style := [fsBold]; // removed underline
   FCssErrorAttri.FrameColor := clRed;

{$if (lcl_major=0) and (lcl_minor=9) and (lcl_release>=31)}
   FCssErrorAttri.FrameEdges := sfeBottom;
   FCssErrorAttri.FrameStyle := slsWaved;
{$endif}
  ...
  FPhpStringSpecialAttri.Foreground := clTeal;
